...
- Avoid using the `service <https://repository.microej.com/modules/ej/library/runtime/service/>`_ library, use singletons or `Constants.getClass()`_ instead. The service library requires embedding class reflection methods and the type names of both interfaces and implementations.
...
The `ej.service.ServiceLoader`_ class of the `service <https://repository.microej.com/modules/ej/library/runtime/service/>`_ library is a dependency injection facility.

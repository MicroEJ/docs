...
This means that for each **exceptions**, including **hardfaults**, a dedicated **exception handler** can be configured.
...
For all hardfault handlers, the information available can and must be printed:
...
Gather all information about the system:
* Processor architecture
* Device memory architecture (flash, ROM, internal, external)
* Device special capabilities (GPU, JPEG decoder, hardware acceleration, peripherals)
* MicroEJ SDK, Architecture and Platform version
* List of native RTOS tasks (numbers, usage, priorities, stacks size)

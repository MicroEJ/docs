...
The library provides a set of functions.
...
The `constant function`_ represents a constant motion that is always at the stop value.
...
It contains a function, a start value, a stop value and a duration.
It proposes a convenient method `Motion.getValue(long)`_ to retrieve the position of the element a the specified elapsed time.

...
#include <stdint.h>
#include <stdbool.h>
#include "intern/LLTRACE_intern.h"
 ...
 * any call to the <code>LLTRACE_record_event_*</code> functions will not
 * record any events.
 * <p>
 * By default, the trace system is stopped.
 */
 ...
 * is the value given to <code>LLTRACE_declare_event_group()</code>).
 * <p>
 * The record is done only if the trace system is started
 * (see <code>LLTRACE_start()</code>).
 *
 * @param group_id
 *            ID of the group returned by <code>LLTRACE_declare_event_group()</code>.
 *
 * @param event_id
 *            ID of the event.
 */

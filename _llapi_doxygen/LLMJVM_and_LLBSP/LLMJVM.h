...
/**
 * Initializes the Core Engine. This function MUST be called once before
 * a call to {@link LLMJVM_start}.
 */
void LLMJVM_initialize(void);

/**
 * Starts the Core Engine and call the <code>main</code> method of  the Java
 ...
 * 
 * @return {@link LLMJVM_E_OK} when the Core Engine ends normally or a negative value
...
/**
 * Prints the state of the Core Engine to the standard output stream.
...
/**
 * Checks internal structure integrity of Core Engine and returns its checksum.<br>
 * If an integrity error is detected, the <code>void LLMJVM_on_CheckIntegrity_error(uint32_t errorCode, void* errorAddress)</code>
 * hook is called and this method returns <code>0</code>.<br>
 ...
 * as it is not possible for the Core Engine to detect the complete memory integrity.<br>
 ...
 * <li>internal structures modified by a <code>LLMJVM</code> function call (e.g., set a pending Java exception, suspend or resume the Java thread, register a resource, ...).</li>
 * </ul>
 * <p>
 ...
 * 
 * @param sniJavaArray is a Java array retrieved from an SNI native arguments.
 ...
 * it in the Core Engine.
 * <p>
 * The Core Engine keeps track of the native resources to prevent any memory leak when the Core
 * Engine or an application is stopped.
 * <p>
 ...
 * by the Kernel or if KF profile is disabled, the <code>close</code> function is called when the Core
 * Engine stops.
 * <p>
 ...
 * 
 * @param close the function called if the Core Engine wants to automatically close the resource.
 * 
 * @param getDescription the function called if the Core Engine wants to retrieve a description of the
 ...
 * function is not called within the Core Engine task or if this function has been called several time in the
 * same native.
 */
 ...
 * has been reclaimed. It will unregister the given native resource so the Core Engine won't call later
 ...
 * is not registered. Returns {@link LLMJVM_E_ERROR} if the function is not called within the Core Engine task.
 */
 ...
 * and the Core Engine or the current application is stopped.
 * <p>
 ...
 * 
 * @return {@link LLMJVM_E_OK} on success. Returns {@link LLMJVM_E_ILLEGAL_ARGUMENT} if <code>close</code> is
 * null. Returns {@link LLMJVM_E_ERROR} if the function is not called within the Core Engine task or if this
 ...
 * the Core Engine task or if no scoped native resource has been previously registered.
 */
 ...
 * This function should be called within the Core Engine task.

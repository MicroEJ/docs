...
.. warning::
   ...
   It is important to note that a VEE Port supporting transitivity is required, so if your VEE Port is:

   - a VEE Port built with SDK 6 ``1.2.0`` or older
   - a VEE Port built with SDK 5

   It is highly recommended to update it to SDK 6 ``1.3.0`` minimum as the feature will be enabled by default in the next SDK 6 major version. 
...
.. warning::
   ...
   It is important to note that a Kernel supporting transitivity is required, so if your Kernel is:

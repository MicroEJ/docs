...
the ``lib3`` library will be added to the runtime classpath of your project, and the ``lib1`` and ``lib2`` libraries will be added to both compile classpath and runtime classpath.
...
When a Kernel is resolved transitively, the Runtime Environment of the Kernel is added to the compile classpath of the project, and the Kernel JAR file and 
its transitive dependencies are added to the runtime classpath of the project.

- Starting from SDK 6 ``1.4.0``, the Runtime Environment of a Kernel is automatically built and published.
- Only the dependencies defined with the ``api`` and ``implementation`` configurations in the Kernel are added to the runtime classpath of the project, 
  so if your Kernel defines the following dependencies::

   dependencies {
      microejVee("org.example:myVeePort:1.0.0")
      api("org.example:lib1:1.0.0")
      implementation("org.example:lib2:1.0.0")
      microejApplication("org.example:myApp:1.0.0")
   }

  the ``lib1`` and ``lib2`` libraries will be added to the runtime classpath.
...
to build the runtime classpath of the testsuites, but the Runtime Environment of the Kernel is not used to compile the project.
...
.. warning::
   ...
   - a Kernel built with SDK 6 ``1.3.0`` or older
   - a Kernel built with SDK 5
   - a local Kernel

   it is highly recommended to update your Kernel to SDK 6 ``1.4.0`` minimum as the feature will be enabled by default in the next SDK 6 major version.

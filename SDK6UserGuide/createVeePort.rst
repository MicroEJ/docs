...
all Libraries provided to the VEE Port with the ``api`` and ``implementation``, so if the VEE Port project does not define dependencies with one of these configurations, 
the published Javadoc Jar is empty. 

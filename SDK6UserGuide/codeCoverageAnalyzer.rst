...
Adapt the values of the ``cc.includes`` and ``cc.excludes`` to your project. The ``cc.excludes`` should contain at least the wrapper class generated by the SDK, which is named after the following pattern: ``<applicationEntryPoint>Wrapper``.

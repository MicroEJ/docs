...
at https://repository.microej.com/modules/com/microej/architecture/ [#note_production]_. 
...
- Browse an ``.xpf`` file or a folder that contains one or more ``.xpf`` files.
...
- Select :guilabel:`File` > :guilabel:`Import` > :guilabel:`MicroEJ` > :guilabel:`Architectures`.
- Browse an ``.xpfp`` file or a folder that contains one or more ``.xpfp`` files.
- Check the :guilabel:`I agree and accept the above terms and conditions...` box to accept the license.
- Click on :guilabel:`Finish` button.
...
selected/deselected from the :guilabel:`Modules` frame.
...
To view all the modules making up a group, click on the Expand All icon on the top-right of the frame. This will let you select/deselect
...
The Configuration project (the project that contains the

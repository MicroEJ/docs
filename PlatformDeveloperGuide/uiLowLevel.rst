...
- ``LLUI_PAINTER_impl.h`` and ``LLDW_PAINTER_impl.h`` list a subpart of UI graphical engine functions to implement (all MicroUI native drawing methods).
- ``ui_drawing.h`` and ``dw_drawing.h`` list all drawing methods the platform can implement.
- ``ui_drawing_soft.h`` and ``dw_drawing_soft.h`` list all drawing methods implemented by the graphical engine.
...
- ``LLUIPainter.java`` and ``LLDWPainter.java`` list a subpart of UI graphical engine functions (all MicroUI native drawing methods).
- ``UIDrawing.java`` and ``DWDrawing.java`` list all drawing methods the platform can implement (and already implemented by the graphical engine).

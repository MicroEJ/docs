...
The loader is automatically used for all fonts retrieved thanks the MicroUI APIs `Font.getAllFonts()`_, `Font.getDefaultFont()`_ and `Font.getFont(String)`_.
...
The Font Loader uses a RAM buffer (External Font Heap) to only contain the font character currently being drawn by the application. 
It is unloaded from RAM when the Graphics Engine's software algorithms no longer need it.

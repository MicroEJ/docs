...
In order to test the Library, it is recommended to create an Application which uses the VEE Port and calls the Sensor Library.
...
In order to validate the Foundation Library on the VEE Ports it will be used, it is recommended to provide a :ref:`validation Testsuite <vee_port_testsuite>`.
It contains a bunch of Java tests which validates the Abstraction Layer that implements the Low Level APIs of the Foundation Library.
...
Libraries can extend the Linking process thanks to LSC files.

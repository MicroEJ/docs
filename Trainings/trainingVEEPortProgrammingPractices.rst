...
- The error code can be anything that helps the developer fix the issue (not necessarily errors documented in the Abstraction Layer API; it can also be a backend error code).
- A log message can be added to specify the error; again, the log string must be wrapped in a macro that can be disabled to save footprint.
- Check the ``SNI_throwNativeException`` function's return code and assert that it is equal to ``SNI_OK``. Return ``SNI_IGNORED_RETURNED_VALUE``.

         ...
         <div class="training_item">
            <a class="training_link" href="gettingStartedIMXRT1170KernelGREEN.html">
               <div class="training_image_container">
                  <ul class="training_image_list">
                     <li class="training_course_number">Course</li>
                     <li class="training_image_list_item">1h</li>
                  </ul>
...
.. raw:: html
      ...
      <script>
        ...
        // Function to increment course numbers
        function incrementCourseNumbers() {
            // Select all .training_items containers
            const trainingItemsContainers = document.querySelectorAll('.training_items');
        ...
        window.onload = incrementCourseNumbers;

         // Function to remove the Training courses on the main page
         const removeChildDivByHref = (href) => {
         const myElement = document.querySelector(`div.training_item > a[href="${href}"]`);
         if (myElement) {
            const childDiv = myElement.closest('div');
            if (childDiv) {
               childDiv.remove();
            }
         }
         ...
         // Function to remove the Training courses in the left menu
         const removeListItemByHref = (href) => {
         const listItem = document.querySelector(`li.toctree-l3 > a[href="${href}"]`);
         if (listItem) {
         ...
         // Check if the 'nxp-mimxrt1170' exists in the URL
         ...
         if (urlParams.has('nxp-mimxrt1170')) {
            ...
            ['trainingSandboxedApps.html'].forEach((href) => {
               ...
               // Remove the link of the STM32 board
               const listItems = document.querySelectorAll('.simple li');

               // Loop through each list item
               listItems.forEach(item => {
                  // Check if the list item contains the specific link text
                  const link = item.querySelector('a');
                  if (link && link.href === "https://www.st.com/en/evaluation-tools/stm32f7508-dk.html") {
                     // Remove the list item if the link matches
                     item.remove();
                  }
               });
